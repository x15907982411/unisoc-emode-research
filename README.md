...
4. **本次测试范围内未发现用户空间 root 路线**：尝试的 10 类攻击面全部失败，评审后补充测试（动态广播 / ContentProvider / Intent 重定向 / 内核版本）亦未发现可行面（测试边界与补充测试见 [docs/03](docs/03-attack-surface.md)）；Mali CVE-2022-38181 路线在本设备适配的 PoC 变体上未走通（BSP 定制差异）；公开可行路线 = **CVE-2022-38694 BROM 解锁 + Magisk**（需 PC + USB）
